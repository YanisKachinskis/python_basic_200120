...
class NotNumberListError(Exception):
    def __init__(self, text='Введен некоректный тип данных. Принимаются только числа. Повторите ввод.'):
        self.text = text
...
while True:
    try:
        key = input(f'Введите {i} элемент списка. Для выхода из программы введите - q: ')
        if key.isdigit():
            my_list.append(key)
            i += 1
        elif key == 'q':
            ...
            raise NotNumberListError()
    except NotNumberListError as error:
        print(error)
print(my_list)
