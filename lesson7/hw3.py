...
class Cell:
    ...
    def __sub__(self, other):
        return Cell(self.partition - other.partition) if self.partition - other.partition > 0 else \
            'Невозможно выполнить вычитание, т.к. количество ячеек первой клетки меньше, чем у второй.'
    ...
    def make_order(self, partitions_in_row):
        row = ''
        for i in range(self.partition // partitions_in_row):
            row += (partitions_in_row * "*") + '\n'
        row += (self.partition % partitions_in_row) * '*'
        return row
